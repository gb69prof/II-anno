const CACHE = 'gbprof-analisi-logica-v1';
...
self.addEventListener('activate',event=>{
  event.waitUntil(Promise.all([
    caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('gbprof-analisi-logica-') && key!==CACHE).map(key=>caches.delete(key)))),
    self.clients.claim()
  ]));
});
