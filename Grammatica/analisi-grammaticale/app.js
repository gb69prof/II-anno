...
(() => {
  ...
  function distinctOrder(list,key) {
    let order=shuffle(list),previous=sessionStorage.getItem(key),signature=order.map(x=>x.id??x).join('|');
    if(order.length>1 && signature===previous){order.push(order.shift());signature=order.map(x=>x.id??x).join('|')}
    sessionStorage.setItem(key,signature);return order;
  }
  ...
  function lesson(id,sourceNo){
    ...
    root.innerHTML=head(c.title,c.summary,'Unità '+String(index+1).padStart(2,'0')+' / '+String(chapters.length).padStart(2,'0'))+
      ...
      c.sources.map(s=>'<details class="source" id="fonte-'+s.number+'"'+(sourceNo==s.number?' open':'')+'><summary>'+esc(s.label)+' · '+esc(s.title)+'</summary><div class="source-body">'+s.html+'</div></details>').join('')+
  ...
  function test(id){
    ...
    const order=distinctOrder(pool,'quiz-order-'+id);
    active={id,order,choices:order.map(q=>distinctOrder([0,1,2],'quiz-opt-'+q.id)),final};
    root.innerHTML=head(final?'Verifica finale':('Verifica · '+c.title),'Osserva gli esempi, interpreta la funzione e poi scegli. La correzione mostra che cosa ripassare.','Test interattivo')+
      ...
      '<div class="actions"><button type="submit">Correggi il test</button><a class="secondary" href="'+(final?'#indice':'#lezione/'+id)+'">Torna alla lezione</a></div></form>';
    document.getElementById('quiz-form').addEventListener('submit',report);
  }
  function report(event){
    event.preventDefault();
    const {id,order,final}=active,form=event.currentTarget;
    ...
    root.innerHTML=head('Risultato del test',final?'Verifica finale':chapter(id).title,'Report e recupero')+
      ...
      '<div class="actions"><a href="#test/'+id+'?ripeti='+Date.now()+'">Riprova il test</a><a class="secondary" href="'+(final?'#indice':'#lezione/'+id)+'">Torna alla lezione</a></div></div></div>';
  }
  function route(){
    ...
    else if(parts[0]==='test')test(parts[1]);
