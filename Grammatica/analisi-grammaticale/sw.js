const CACHE = 'gbprof-analisi-grammaticale-v2';
...
self.addEventListener('activate',event=>{
  event.waitUntil(Promise.all([
    caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('gbprof-analisi-grammaticale-') && key!==CACHE).map(key=>caches.delete(key)))),
    self.clients.claim()
  ]));
});
